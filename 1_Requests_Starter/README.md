## Udacity Bookshelf
    This is a virtual bookshelf developed as part of the Udacity API development and Documentation course.
    In this API, students are able to add books to the bookshelf, give them a rating, update the rating and search for books of
    choice through the list of books.
    The ultimate goal of the project is to demonstrate an understanding of structuring and implementing well-formatted API endpoints.
...
Verify that the database user in the `/backend/books.psql`, `/backend/models.py`, and `/backend/test_flaskr.py` files must be either the `student` or `postgres` (default username). The `student`/`student` user credentials are used, whereas, the local implementation can use the dafault `postgres` user without a password as well. (See the `/backend/setup.sql` for more details!)

2. **Create the database and a user**<br>
In your terminal, navigate to the */Bookshelf/1_Requests_Starter/backend/* directory, and run the following:
```bash
cd Bookshelf/1_Requests_Starter/backend
...
### Getting Started
-Base URL: This app is currently only run locally and is not hosted as a base URL. The backend app is hosted at the default `http://127.0.0.1:5000/`, which is set as a proxy in the frontend configuration.
...
- General:
    ...
    - Results are paginated in groups of 8. Include a request argument to choose a page number, starting from 1.
...
-  `curl http://127.0.0.1:5000/books?page=3 -X POST -H "Content-Type: application/json" -d '{"title": "The 100", "author":"Neil Alvin", "rating": "4"}'`
